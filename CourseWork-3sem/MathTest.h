...
{
protected:
	std::vector<size_t> user_answers; //Вектор хранит ответы пользователя //Ответ - это номер из списка ответов
	std::string path{"MathTests.bin"}; //Путь к файлу с математическими тестами
public:
	MathTest() : TestRating() {};
	~MathTest() = default;
	...
	virtual void check(); //Сверяем ответы пользователя с правильными ответами и выводим результат
