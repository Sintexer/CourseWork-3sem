...
void PhysTest::MakeTable() { //Выводит ответы пользователя на тест в виде таблицы
	...
	if (questions.size())
		for (qstit, vc; qstit != questions.end(); ++qstit, ++vc) {
			temp = std::to_string(vc + 1) + "|" + toStr(user_answers[vc]) + "|"; //Вносит в строку таблицы номер вопроса и ответ пользователя
...
void PhysTest::start(){ //Проводит тест
	...
	while (it != questions.end()) {
		...
		cout << "\n" << "1: Прервать тест\n"
			 << "0: Назад" 
			 << "\nВведите ваш ответ: \n" << endl;
		inputSafe(cin, answer); //Безопасный ввод числа
		...
		if (answer == 1) { //Если пользователь выбрал "Прервать тест"
			if (user_answers.size()) { //Если пользователь ответил хотя бы на 1 вопрос
				...
				cout << "Желаете сохранить ваш результат?\n"
					...
					<< "2: Нет\n" << endl
					<< "0: Назад" << endl;
				inputSafe(cin, ans, 0, 2); //Безопасный ввод числа в диапазоне от 0 до 2
				switch (ans){
				case 1:
...
void PhysTest::check(){ //Проверка теста по правильным ответам
...
void PhysTest::result(){ //Выводит процент правильных ответов на экран
...
std::istream& operator>>(std::istream& in, PhysTest& obj){ //Ввод теста из потока
